...
{
    // KOOK: discriminator field is 'identify_num'. Accept either.
    bool got_name = utils::TryGetJsonValue(data, m_Username, "username");
    std::string disc_tmp;
    bool got_disc = utils::TryGetJsonValue(data, disc_tmp, "discriminator");
    if (!got_disc)
        got_disc = utils::TryGetJsonValue(data, disc_tmp, "identify_num");
    if (got_disc)
        m_Discriminator = std::move(disc_tmp);

    // Consider user valid as long as we have username and id; discriminator may be empty on some payloads
    _valid = got_name;
	...
	{
		pawn_cb::Error error;
		pawn_cb::Callback::CallFirst(error, "KCC_OnUserUpdate", GetPawnId());
	}
}
...
void UserManager::Initialize()
{
	...
	Network::Get()->WebSocket().RegisterEvent(WebSocket::Event::READY, [this](json const &data)
	{
		...
		{
			AddUser(data["user"]); // bot self
		}
		else
		{
			// KOOK READY may not include 'user'; do not block initialization.
			Logger::Get()->Log(samplog_LogLevel::DEBUG,
				"READY has no 'user'; skipping self user warm-up.");
		}
		m_Initialized++;
	});
...
/*
	Network::Get()->WebSocket().RegisterEvent(WebSocket::Event::USER_UPDATE, [](json const &data)
	{
		...
		PawnDispatcher::Get()->Dispatch([data, user_id]() mutable
		{
			...
			// forward KCC_OnUserUpdate(DCC_User:user);
			pawn_cb::Error error;
			pawn_cb::Callback::CallFirst(error, "KCC_OnUserUpdate", user->GetPawnId());
		});
	});*/
}
    ...
    std::string const &name, std::string const &discriminator)
{
    ...
    for (auto const &u : m_Users)
    {
        ...
        if (user->GetUsername().compare(name) == 0
            && (discriminator.empty() || user->GetDiscriminator().compare(discriminator) == 0))
        {
            return user;
        }
    }
