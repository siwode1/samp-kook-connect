...
private: // variables
	...
	std::string _gatewayUrl; // host only
	std::string _gatewayPath; // e.g. "/gateway"
...
private: // functions
	void Initialize(std::string token, std::string gateway_host, std::string gateway_path);
	...
	// KOOK uses token in WebSocket handshake URL, not identify payload
	// These methods are kept for API compatibility but are no-op
	void Identify() { /* KOOK: token in handshake URL, no identify payload */ }
	void SendResumePayload() { /* KOOK: token in handshake URL, no resume payload */ }
